...
class CustomizedDist(Distribution):
    ...
    def fetch_build_egg(self, req):
        """Specialized version of Distribution.fetch_build_egg that respects
        respects allow_hosts and index_url."""
...
class PyTest(orig.test):
    ...
    def ensure_setuptools_version():
        """Due to the fact that pytest-runner is often required (via setup-
        requires directive) by toolchains that never invoke it (i.e. they're
        only installing the package, not testing it), instead of declaring the
        dependency in the package metadata, assert the requirement at run
        time."""
    ...
    def marker_passes(marker):
        """Given an environment marker, return True if the marker is valid and
    ...
    def install_extra_dists(self, dist):
        """Install extras that are indicated by markers or install all extras
        if '--extras' is indicated."""
    ...
    def run(self):
        """Override run to ensure requirements are available in this session
        (but don't install them anywhere)."""
